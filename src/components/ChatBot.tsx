...
// FIX 1: Moved DesktopChat OUTSIDE of ChatBot component
// FIX 2: Now receives all needed data and handlers as props
const DesktopChat = ({ messages, inputText, setInputText, handleSendMessage, handleKeyPress, setIsOpen }: {
  messages: Message[];
  inputText: string;
...
// FIX 1: Moved MobileChat OUTSIDE of ChatBot component
// FIX 2: Now receives all needed data and handlers as props
const MobileChat = ({ messages, inputText, setInputText, handleSendMessage, handleKeyPress, setIsOpen }: {
  messages: Message[];
  inputText: string;
  setInputText: (value: string) => void;
  handleSendMessage: () => void;
...
const ChatBot = () => {
  ...
  // FIX 3: Added mounted state to prevent hydration mismatch with createPortal
  ...
  // FIX 4: Replaced deprecated onKeyPress with onKeyDown
  ...
  return (
    <>
      ...
      {/* FIX 3: Use mounted state instead of typeof window check
          This prevents hydration mismatch between server and client render */}
      <AnimatePresence>
        {isOpen && mounted && (
          <>
            {isMobile ? (
              createPortal(<MobileChat {...chatProps} />, document.body)
            ) : (
